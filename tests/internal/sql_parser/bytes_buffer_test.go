...
func check(t *testing.T, testname string, buf *tokenizer.BytesBuffer, s string) {
	bytes := buf.Bytes()
	str := buf.String()
...
func fillString(t *testing.T, testname string, buf *tokenizer.BytesBuffer, s string, n int, fus string) string {
...
// The initial contents of buf corresponds to the string s;
// the result is the final contents of buf returned as a string.
func fillBytes(t *testing.T, testname string, buf *tokenizer.BytesBuffer, s string, n int, fub []byte) string {
	check(t, testname+" (fill 1)", buf, s)
	for ; n > 0; n-- {
		m, err := buf.Write(fub)
		if m != len(fub) {
...
func TestNewBuffer(t *testing.T) {
	buf := tokenizer.NewBytesBuffer(testBytes)
	check(t, "NewBuffer", buf, testString)
}

var buf tokenizer.BytesBuffer

// Calling tokenizer.NewBytesBuffer and immediately shallow copying the tokenizer.BytesBuffer struct
// should not result in any allocations.
// This can be used to reset the underlying []byte of an existing tokenizer.BytesBuffer.
...
func TestNewBufferString(t *testing.T) {
	buf := tokenizer.NewBytesBufferString(testString)
	check(t, "NewBufferString", buf, testString)
}

// Empty buf through repeated reads into fub.
// The initial contents of buf corresponds to the string s.
func empty(t *testing.T, testname string, buf *tokenizer.BytesBuffer, s string, fub []byte) {
...
func TestCapWithPreallocatedSlice(t *testing.T) {
	buf := tokenizer.NewBytesBuffer(make([]byte, 10))
	n := buf.Cap()
	if n != 10 {
...
func TestCapWithSliceAndWrittenData(t *testing.T) {
	buf := tokenizer.NewBytesBuffer(make([]byte, 0, 10))
	buf.Write([]byte("test"))
	n := buf.Cap()
	if n != 10 {
		t.Errorf("expected 10, got %d", n)
...
// Make sure that an empty tokenizer.BytesBuffer remains empty when
...
func TestReadFromPanicReader(t *testing.T) {
	...
	var buf2 tokenizer.BytesBuffer
	defer func() {
		recover()
...
func TestReadFromNegativeReader(t *testing.T) {
	...
	defer func() {
		...
		case error:
			...
			wantError := "tokenizer.BytesBuffer: reader returned negative count from Read"
	...
	for i := 0; i <= 5; i++ {
		for j := i; j <= 5; j++ {
			for k := 0; k <= 6; k++ {
				...
				buf := tokenizer.NewBytesBuffer(b[0:j])
...
func TestReadBytes(t *testing.T) {
	for _, test := range readBytesTests {
		buf := tokenizer.NewBytesBufferString(test.buffer)
		var err error
...
func BenchmarkReadString(b *testing.B) {
	...
	b.SetBytes(int64(n))
	for i := 0; i < b.N; i++ {
		buf := tokenizer.NewBytesBuffer(data)
...
func TestGrow(t *testing.T) {
	...
	for _, growLen := range []int{0, 100, 1000, 10000, 100000} {
		for _, startLen := range []int{0, 100, 1000, 10000, 100000} {
			...
			buf := tokenizer.NewBytesBuffer(xBytes)
...
func TestGrowOverflow(t *testing.T) {
	...
	buf := tokenizer.NewBytesBuffer(make([]byte, 1))
...
func TestBufferGrowth(t *testing.T) {
	...
	cap1 := b.Cap()
	// (*tokenizer.BytesBuffer).grow allows for 2x capacity slop before sliding,
...
func BenchmarkWriteByte(b *testing.B) {
	const n = 4 << 10
	b.SetBytes(n)
	buf := tokenizer.NewBytesBuffer(make([]byte, n))
	for i := 0; i < b.N; i++ {
		buf.Reset()
		for i := 0; i < n; i++ {
			buf.WriteByte('x')
...
func BenchmarkWriteRune(b *testing.B) {
	...
	buf := tokenizer.NewBytesBuffer(make([]byte, n*utf8.UTFMax))
	for i := 0; i < b.N; i++ {
		buf.Reset()
		for i := 0; i < n; i++ {
			buf.WriteRune(r)
