...
// NewSequenceIdent creates a new SequenceIdent.
func NewSequenceIdent(str string) SequenceIdent {
	// Use StringClone on the table name to ensure it is not pinned to the
	// underlying query string that has been generated by the parser. This
	// could lead to a significant increase in memory usage when the table
	// name comes from a large query.
	return SequenceIdent{V: strings.Clone(str)}
}

// IsEmpty returns true if TabIdent is empty.
func (node SequenceIdent) IsEmpty() bool {
	return node.V == ""
}
...
// instead. The Stringer conformance is for usage
// in templates.
func (node SequenceIdent) String() string {
	return node.V
}

// CompliantName returns a compliant id name
// that can be used for a bind var.
func (node SequenceIdent) CompliantName() string {
	return compliantName(node.V)
}

// MarshalJSON marshals into JSON.
func (node SequenceIdent) MarshalJSON() ([]byte, error) {
	return json.Marshal(node.V)
}

// UnmarshalJSON unmarshals from JSON.
func (node *SequenceIdent) UnmarshalJSON(b []byte) error {
	var result string
	err := json.Unmarshal(b, &result)
	if err != nil {
		return err
