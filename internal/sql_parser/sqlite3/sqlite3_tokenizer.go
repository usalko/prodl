...
// Sqlite3Tokenizer is the struct used to generate SQL
...
var zeroParser sqlite3ParserImpl

// sqlite3ParsePooled is a wrapper around sqlite3Parse that pools the parser objects. There isn't a
// particularly good reason to use sqlite3Parse directly, since it immediately discards its parser.
//
...
func ParsePooled(lexer tokenizer.Tokenizer) int {
	parser := parserPool.Get().(*sqlite3ParserImpl)
	defer func() {
		*parser = zeroParser
		parserPool.Put(parser)
	}()
	return parser.Parse(lexer.(sqlite3Lexer))
}

func Parse(lexer tokenizer.Tokenizer) int {
	return sqlite3Parse(lexer.(sqlite3Lexer))
}

// SQLITE3ServerVersion is what will present as it's version during the connection handshake,
// and as the value to the @@version system variable. If nothing is provided, will report itself as
// a specific SQLITE3 version with the vitess version appended to it
var SQLITE3ServerVersion = flag.String("sqlite3_server_version", "", "SQLITE3 server version to advertise.")

// NewSqlite3StringTokenizer creates a new Tokenizer for the
// sql string.
func NewSqlite3StringTokenizer(sql string) *Sqlite3Tokenizer {

	return &Sqlite3Tokenizer{
		buf:      sql,
...
func (tkn *Sqlite3Tokenizer) Scan() (int, string) {
	if tkn.specialComment != nil {
		...
		// for scanning such kind of comment: /*! SQLITE3-specific code */
		...
		case '/':
			...
			case '*':
				...
				if tkn.Cur() == '!' && !tkn.SkipSpecialComments {
					tkn.Skip(1)
					return tkn.scanSQLITE3SpecificComment()
				}
...
func (tkn *Sqlite3Tokenizer) skipStatement() int {
	tkn.SkipToEnd = false
	for {
		typ, _ := tkn.Scan()
		if typ == 0 || typ == ';' || typ == LEX_ERROR {
...
func (tkn *Sqlite3Tokenizer) scanIdentifier(isVariable bool) (int, string) {
	start := tkn.Pos
	tkn.Skip(1)
	...
	if keywordID, found := cache.KeywordLookup(keywordName, dialect.SQLITE3); found {
...
func (tkn *Sqlite3Tokenizer) scanHex() (int, string) {
	start := tkn.Pos
	tkn.scanMantissa(16)
...
func (tkn *Sqlite3Tokenizer) scanBitLiteral() (int, string) {
	start := tkn.Pos
	tkn.scanMantissa(2)
...
// so far.
func (tkn *Sqlite3Tokenizer) scanLiteralIdentifierSlow(buf *strings.Builder) (int, string) {
	backTickSeen := true
	for {
		if backTickSeen {
			if tkn.Cur() != '`' {
...
func (tkn *Sqlite3Tokenizer) scanLiteralIdentifier() (int, string) {
	start := tkn.Pos
	for {
		switch tkn.Cur() {
		case '`':
...
func (tkn *Sqlite3Tokenizer) scanBindVar() (int, string) {
	start := tkn.Pos
	token := VALUE_ARG
...
func (tkn *Sqlite3Tokenizer) scanMantissa(base int) {
...
func (tkn *Sqlite3Tokenizer) scanNumber() (int, string) {
	start := tkn.Pos
	token := INTEGRAL
...
// is started with '//', '--' or '#'.
func (tkn *Sqlite3Tokenizer) scanCommentType1(prefixLen int) (int, string) {
...
func (tkn *Sqlite3Tokenizer) scanCommentType2() (int, string) {
	start := tkn.Pos - 2
	for {
		if tkn.Cur() == '*' {
			tkn.Skip(1)
...
// scanSQLITE3SpecificComment scans a SQLITE3 comment pragma, which always starts with '//*`
func (tkn *Sqlite3Tokenizer) scanSQLITE3SpecificComment() (int, string) {
	start := tkn.Pos - 3
	for {
		if tkn.Cur() == '*' {
			tkn.Skip(1)
	...
	commentVersion, sql := ExtractSqlite3Comment(tkn.buf[start:tkn.Pos])

	if "1" >= commentVersion {
		// Only add the special comment to the tokenizer if the version of SQLITE3 is higher or equal to the comment version
		tkn.specialComment = NewSqlite3StringTokenizer(sql)
	}
...
// ExtractSqlite3Comment extracts the version and SQL from a comment-only query
